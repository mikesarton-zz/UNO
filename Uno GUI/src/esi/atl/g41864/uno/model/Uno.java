...
public class Uno implements Observable {
     ...
     *
     * @param p The list with the player's names.
     * @throws UnoException If the game is running or if a name is empty.
     */
    public Uno(List<String> p) throws UnoException {
        if (state != StateOfGame.STOPPED) {
            throw new UnoException("Uno() ne peut pas être appelé si le jeu"
                    + " n'est pas en train de tourner.");
        }

        if (p.size() < 2 || p.size() > 10) {
            throw new UnoException("Nombre de joueur invalide : il faut "
                    + "minimum 2 joueurs.");
        }

        for (String str : p) {
            if (!str.equalsIgnoreCase("")) {
                if (str.substring(0, 2).equals("IA")) {
                    players.add(new Player(str, true));
                } else {
                    players.add(new Player(str, false));
                }
            } else {
                throw new UnoException("Nom de joueur invalide : aucun nom"
                        + " de joueur ne peut être vide.");
            }
        }
